        ...
        IGetAllSkillsBySkillNameMiddleware
{
    ...
    )
    {
        ...
        var cachedKey =
            $"{nameof(GetAllSkillsBySkillNameRequest)}_param_{request.SkillName.ToLower()}";
        ...
        if (
            !Equals(objA: cacheModel, objB: AppCacheModel<GetAllSkillsBySkillNameResponse>.NotFound)
        )
        {
