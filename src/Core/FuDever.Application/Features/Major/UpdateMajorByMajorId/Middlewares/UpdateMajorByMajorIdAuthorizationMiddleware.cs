...
internal sealed class UpdateMajorByMajorIdAuthorizationMiddleware
    : IPipelineBehavior<UpdateMajorByMajorIdRequest, UpdateMajorByMajorIdResponse>,
        IUpdateMajorByMajorIdMiddleware
{
    private readonly ISender _sender;

    public UpdateMajorByMajorIdAuthorizationMiddleware(ISender sender)
    {
        _sender = sender;
    }
    ...
    )
    {
        ...
        foreach (var requirement in requirements)
        {
            ...
            {
                return new() { StatusCode = UpdateMajorByMajorIdResponseStatusCode.FORBIDDEN };
            }

            if (!result.IsAuthorized)
            {
                return new() { StatusCode = UpdateMajorByMajorIdResponseStatusCode.UN_AUTHORIZED };
            }
        }
