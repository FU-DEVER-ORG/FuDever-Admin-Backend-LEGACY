...
internal sealed class GetAllTemporarilyRemovedPositionsAuthorizationMiddleware
    : IPipelineBehavior<
        ...
        IGetAllTemporarilyRemovedPositionsMiddleware
{
    ...
    public GetAllTemporarilyRemovedPositionsAuthorizationMiddleware(ISender sender)
    {
        _sender = sender;
    }
    ...
    )
    {
        ...
        foreach (var requirement in requirements)
        {
            ...
            if (result.IsForbidden)
            {
                return new()
                {
                    StatusCode = GetAllTemporarilyRemovedPositionsResponseStatusCode.FORBIDDEN
                };
            }
            ...
            if (!result.IsAuthorized)
            {
                return new()
                {
                    StatusCode = GetAllTemporarilyRemovedPositionsResponseStatusCode.UN_AUTHORIZED
                };
            }
        }
