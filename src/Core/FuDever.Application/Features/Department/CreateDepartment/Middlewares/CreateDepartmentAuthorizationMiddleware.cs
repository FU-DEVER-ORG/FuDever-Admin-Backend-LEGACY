...
/// <summary>
///     Create department request authorization middleware.
/// </summary>
[FeatureMiddlewareOrder(value: 1)]
internal sealed class CreateDepartmentAuthorizationMiddleware
    : IPipelineBehavior<CreateDepartmentRequest, CreateDepartmentResponse>,
        ICreateDepartmentMiddleware
{
    private readonly ISender _sender;

    public CreateDepartmentAuthorizationMiddleware(ISender sender)
    {
        _sender = sender;
    }
    ...
    )
    {
        ...
        foreach (var requirement in requirements)
        {
            ...
            {
                return new() { StatusCode = CreateDepartmentResponseStatusCode.FORBIDDEN };
            }

            if (!result.IsAuthorized)
            {
                return new() { StatusCode = CreateDepartmentResponseStatusCode.UN_AUTHORIZED };
            }
        }
