...
/// <summary>
///     Get all users request caching middleware.
/// </summary>
[FeatureMiddlewareOrder(value: 2)]
internal sealed class GetAllUsersCachingMiddleware
    : IPipelineBehavior<GetAllUsersRequest, GetAllUsersResponse>,
        IGetAllUsersMiddleware
{
    private readonly ICacheHandler _cacheHandler;

    public GetAllUsersCachingMiddleware(ICacheHandler cacheHandler)
    {
    ...
    )
    {
        ...
        const string cachedKey = nameof(GetAllUsersRequest);

        // Retrieve from cache.
        var cacheModel = await _cacheHandler.GetAsync<GetAllUsersResponse>(
            key: cachedKey,
            cancellationToken: cancellationToken
        );

        // Cache value does not exist.
        if (!Equals(objA: cacheModel, objB: AppCacheModel<GetAllUsersResponse>.NotFound))
        {
